<template>
  <div class="ext-panel" :class="{moving : moving}" :style="{top: `${top}px`, left: `${left}px`}" ref="editor" >
    ...
    <div class="editor" :style="{height: `${height}px`}" v-show="showSidebar">
      <y-tabs v-model="activeTab" @tab-click="onTabClick">
        <y-tab-pane label="Config" name="config">
          <theme-configurator
        ...
        <y-tab-pane label="Gallery" name="gallery">
          <gallery 
            ref='gallery'
            :height="height"
            :width="width - 7"
...
export default {
  ...
  mounted() {
    ...
    // chrome.storage.local.remove('YUN_THEME_USER_CONFIG');
...
<style>
.ext-panel  .editor .y-tabs__content, .ext-panel  .editor .y-tabs--top, .ext-panel  .editor .y-tab-pane {
  height: 100%;
}
.ext-panel .y-tabs__nav-scroll >div {
