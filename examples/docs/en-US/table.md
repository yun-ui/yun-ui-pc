...
:::demo Use `row-class-name` in `y-table` to add custom classes to a certain row. Then you can style it with custom classes.
```html
<template>
  <y-table
    :data="tableData"
...
:::demo By setting the attribute `height` of `y-table`, you can fix the table header without any other codes.
```html
<template>
  <y-table
    :data="tableData"
...
:::demo Attribute `fixed` is used in `y-table-column`, it accepts a `Boolean`. If `true`, the column will be fixed at left. It also accepts two string literals: 'left' and 'right', both indicating that the column will be fixed at corresponding direction.
```html
<template>
  <y-table
    :data="tableData"
...
:::demo  By setting the attribute `max-height` of `y-table`, you can fix the table header. The table body scrolls only if the height of the rows exceeds the max height value.
```html
<template>
  <y-table
    :data="tableData"
...
:::demo Only need to place y-table-column inside a y-table-column, you can achieve group header.
```html
<template>
  <y-table
    :data="tableData"
    ...
    <y-table-column label="Delivery Info">
      <y-table-column
        prop="name"
        label="Name"
        width="120">
      </y-table-column>
      <y-table-column label="Address Info">
        <y-table-column
          prop="state"
          label="State"
          width="120">
...
<template>
  ...
  <div style="margin-top: 20px">
    <y-button @click="setCurrent(tableData[1])">Select second row</y-button>
    <y-button @click="setCurrent()">Clear selection</y-button>
  </div>
</template>
...
<template>
  ...
  <div style="margin-top: 20px">
    <y-button @click="toggleSelection([tableData[1], tableData[2]])">Toggle selection status of second and third rows</y-button>
    <y-button @click="toggleSelection()">Clear selection</y-button>
  </div>
</template>
...
:::demo Set attribute `filters` and `filter-method` in `y-table-column` makes this column filterable. `filters` is an array, and `filter-method` is a function deciding which rows are displayed. It has three parameters: `value`, `row` and `column`.
```html
<template>
  <y-button @click="resetDateFilter">reset date filter</y-button>
  <y-button @click="clearFilter">reset all filters</y-button>
  <y-table
    ref="filterTable"
    :data="tableData"
    style="width: 100%">
    ...
    <y-table-column
      ...
      <template slot-scope="scope">
        <y-tag
          ...
          disable-transitions>{{scope.row.tag}}</y-tag>
      </template>
    </y-table-column>
  </y-table>
</template>
...
<template>
  <y-table
    ...
    <y-table-column
      ...
      width="180">
      <template slot-scope="scope">
        <y-popover trigger="hover" placement="top">
          <p>Name: {{ scope.row.name }}</p>
          <p>Addr: {{ scope.row.address }}</p>
          <div slot="reference" class="name-wrapper">
            <y-tag size="medium">{{ scope.row.name }}</y-tag>
          </div>
        </y-popover>
      </template>
    </y-table-column>
...
:::demo Activate expandable row by adding type="expand" and scoped slot. The template for y-table-column will be rendered as the contents of the expanded row, and you can access the same attributes as when you are using `Scoped slot` in custom column templates.
```html
<template>
  <y-table
    :data="tableData"
...
:::demo To customize row indices, use `index` attribute on `y-table-column` with `type=index`. If it is assigned to a number, all indices will have an offset of that number. It also accepts a method with each index (starting from `0`) as parameter, and the returned value will be displayed as index.
...
| resizable | whether column width can be resized, works when `border` of `y-table` is `true` | boolean | — | false |
