## Custom theme
Yun uses BEM-styled CSS so that you can override styles easily. But if you need to replace styles at a large scale, e.g. change the theme color from blue to orange or green, maybe overriding them one by one is not a good idea. We provide four ways to change the style variables.
...
Also, use [Theme Roller Chrome Extension](https://chrome.google.com/webstore/detail/element-theme-roller/lifkjlojflekabbmlddfccdkphlelmim)，to customize theme and preview in real-time on any website developed by Yun.
...
If you just want to change the theme color of Yun, the [theme preview website](https://elementui.github.io/theme-chalk-preview/#/en-US) is recommended. The theme color of Yun is bright and friendly blue. By changing it, you can make Yun more visually connected to specific projects.
...
`theme-chalk` is written in SCSS. If your project also uses SCSS, you can directly change Yun style variables. Create a new style file, e.g. `element-variables.scss`:
...
$--font-path: '~yun-ui-pc/lib/theme-chalk/fonts';

@import "~yun-ui-pc/packages/theme-chalk/src/index";
```

Then in the entry file of your project, import this style file instead of Yun's built CSS:
```JS
import Vue from 'vue'
...
:::tip
Note that it is required to override icon font path to the relative path of Yun's font files.
:::
...
# from GitHub
npm i https://github.com/YunUI/theme-chalk -D
```
...
In `element-variables.scss` you can find all the variables we used to style Yun and they are defined in SCSS format. Here's a snippet:
```css
...
If you are unfamiliar with `babel-plugin-component`, please refer to <a href="./#/en-US/component/quickstart">Quick Start</a>. For more details, check out the [project repository](https://github.com/YunUI/element-theme) of `element-theme`.