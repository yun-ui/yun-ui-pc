...
<template>
  <div class="page-container page-theme">
    ...
    <y-dialog :visible.sync="copyDialogVisible">
      <y-form :model="copyForm" ref="copyForm" :rules="copyFormRule">
        <y-form-item label="<%= 3 >" prop="name">
          <y-input v-model="copyForm.name"></y-input>
        </y-form-item>
      </y-form>
      <div slot="footer" class="dialog-footer">
        <y-button @click="closeCopyForm">{{getActionDisplayName('cancel')}}</y-button>
        <y-button type="primary" @click="copyToUser">{{getActionDisplayName('confirm')}}</y-button>
      </div>
    </y-dialog>
  </div>
</template>
