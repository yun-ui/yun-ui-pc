...
- Transfer
  - Fix incorrect line-height of y-transfer's first list item when it was used with  y-form-item (#18917 by @Hanx)
- InputNumber
...
- Calendar
  - Import y-button and y-button-group (#17376 by @masongzhi)
- MessageBox
...
- Theme
  - Add Chrome Extension: Yun Theme Extension (#16686 by @iamkun)
- Icon
...
- Tree
  - Make y-tree generic (#15934 by @JeremyWuuuuu)
...
- Input
  - Fix show-word-count style in y-form (#15359 by @lvjiaxuan)
...
- Switch
  - Label with y-switch repeating event (#15178 by @FAKER-A)
- Slider
...
- Form
  - Fix switching the rules in y-form not work (#14985 by @luckyCao)
...
- NavMenu
  - Fix click y-submenu trigger childMenu pop again bug (#14443 by @PanJiaChen)
- Dropdown
...
- DropdownItem
  - Add icon prop to `y-dropdown-item` (by @gabrielboliveira in #14088)
- Input
...
- Menu
  - Add support for `y-menu-item` without index (by @georgyfarniev in #13298)
- Table
...
- Build
  - Minimizing css and js for Yun doc site (by @iamkun in #14430)
...
- MessageBox
  - Fixed the `y-popup-parent--hidden` class not removed when opening MessageBox in succession, #11371
...
- Added `shadow` attribute for Card, #10418 (by @ElementYouJun)
...
- Progress
  - Added `color` attribute, #10352 (by @ElementYouJun)
- Button
  - Added `circle` attribute, #10359 (by @ElementYouJun)
...
- General
  ...
  - Now you can configure component sizes globally. When importing Yun, you can add a global config object with a `size` prop to configure default sizes for all components.
- Button
