...
- Calendar
  - 导入 y-button 和 y-button-group (#17376 by @masongzhi)
- MessageBox
...
- Theme
  - 增加 Chrome 插件: Yun Theme Extension (#16686 by @iamkun)
- Icon
...
- NavMenu
  - 修复点击 y-submenu 多次触发 childMenu 问题 (#14443 by @PanJiaChen)
- Dropdown
...
- DropdownItem
  - `y-dropdown-item` 支持添加 icon (by @gabrielboliveira in #14088)
- Input
...
- Menu
  - 支持 `y-menu-item` 不添加 index (by @georgyfarniev in #13298)
- Table
...
- Docs
  ...
  - 添加 `y-input` 是受控组件的警示 (by @wacky6 in #14463)
...
- Build
  - 压缩 Yun 文档站的 JS 和 CSS 文件 (by @iamkun in #14430)
...
#### 修复
- 修复升级 Webpack 4 产生的问题，无法具名 `import` 组件，`YUN.locale()` 调用报错。（by @island205 in #14220）
...
- MessageBox
  - 修复连续打开两个 MessageBox 时 `y-popup-parent--hidden` 无法移除的问题，#11371
...
- 新增 Card 的 `shadow` 属性，#10418（by @ElementYouJun）
...
- Progress
  - 新增 `color` 属性，#10352（by @ElementYouJun）
- Button
  - 新增 `circle` 属性，#10359（by @ElementYouJun）
...
- 综合
  ...
  - 新增全局配置组件尺寸的功能。在引入 Yun 时，配置 `size` 字段可以改变所有组件的默认尺寸
- Button
