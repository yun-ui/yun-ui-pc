...
describe('Select', () => {
  ...
  it('create', () => {
    ...
    expect(vm.$el.className).to.equal('y-select');
    expect(vm.$el.querySelector('.y-input__inner').placeholder).to.equal('请选择');
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.y-select-dropdown__item');
  ...
  it('custom dropdown class', () => {
    ...
    const dropdown = vm.$el.querySelector('.y-select-dropdown');
  ...
  it('default value', done => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.y-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('single select', done => {
    vm = createVue({
      template: `
        <div>
          <y-select v-model="value" @change="handleChange">
            <y-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('disabled select', () => {
    ...
    expect(vm.$el.querySelector('.y-input').classList.contains('is-disabled')).to.true;
  });
  ...
  it('visible event', done => {
    vm = createVue({
      template: `
        <div>
          <y-select v-model="value" @visible-change="handleVisibleChange">
            <y-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('clearable', done => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector('.y-input__icon.y-icon-circle-close');
  ...
  it('object typed value', done => {
    vm = createVue({
      template: `
        <div>
          <y-select v-model="value" value-key="id">
            <y-option
              v-for="item in options"
              :label="item.label"
              :key="item.id"
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.y-input__inner').value).to.equal('label1');
      expect(vm.$el.querySelector('.y-select-dropdown__item').classList.contains('selected'));
      done();
    }, 100);
  });
  ...
  it('prefixed icon', () => {
      ...
      template: `
        <div>
          <y-select v-model="value">
            ...
            <i slot="prefix" class="y-input__icon y-icon-search"></i>
          </y-select>
        </div>
      `,
    ...
    expect(vm.$el.querySelector('.y-input__icon').classList.contains('y-icon-search')).to.be.true;
  });

  it('custom y-option template', () => {
    vm = createVue({
      template: `
        <div>
          <y-select v-model="value">
    ...
    expect(vm.$el.querySelector('.y-select-dropdown__item p').textContent).to.equal('label value');
  });
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.y-select-group__wrap');
    const options = groups[1].querySelectorAll('.y-select-dropdown__item');
  ...
  it('allow create', done => {
    ...
    setTimeout(() => {
      ...
      select.onInputChange();
      setTimeout(() => {
        const options = document.querySelectorAll('.y-select-dropdown__item span');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.y-tag__close');
  ...
  it('multiple remove-tag', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <y-select v-model="value" multiple @remove-tag="handleRemoveTag">
            <y-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('should return focus to input inside select after option select', done => {
    vm = createVue({
      template: `
        <div>
          <y-select v-model="value" ref="select">
            <y-option label="1" :value="1" />
          </y-select>
        </div>
      `,
      data() {
    ...
    const option = vm.$el.querySelectorAll('.y-select-dropdown__item')[0];
  ...
  it('only emit change on user input', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <y-select v-model="value" @change="change" ref="select">
            <y-option label="1" :value="1" />
            <y-option label="2" :value="2" />
            <y-option label="3" :value="3" />
          </y-select>
        </div>
      `,
      data() {
  ...
  it('should set placeholder to label of selected option when filterable is true and multiple is false', async() => {
    vm = createVue({
      template: `
        <div>
          <y-select ref="select" v-model="value" filterable>
            <y-option label="test" value="test" />
          </y-select>
        </div>
      `,
      data() {
    ...
    expect(vm.$el.querySelector('.y-input__inner').placeholder).to.be.equal('test');
  ...
  it('default value is null or undefined', async() => {
    ...
    vm.value = null;
    await waitImmediate();
    expect(vm.$el.querySelector('.y-input__inner').value).to.equal('');
    vm.value = '选项1';
    await waitImmediate();
    expect(vm.$el.querySelector('.y-input__inner').value).to.equal('黄金糕');
  });
