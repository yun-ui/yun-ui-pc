...
describe('Loading', () => {
  ...
  describe('as a directive', () => {
    it('create', done => {
      ...
      Vue.nextTick(() => {
        const mask = vm.$el.querySelector('.y-loading-mask');
    ...
    it('unbind', done => {
      ...
      Vue.nextTick(() => {
        ...
        Vue.nextTick(() => {
          ...
          vm2.show = false;
          Vue.nextTick(() => {
            expect(document.querySelector('.y-loading-mask')).to.not.exist;
            done();
          });
        });
      });
    ...
    it('body', done => {
      ...
      Vue.nextTick(() => {
        const mask = document.querySelector('.y-loading-mask');
    ...
    it('lock', done => {
      ...
      Vue.nextTick(() => {
        ...
        vm.loading = false;
        document.body.removeChild(document.querySelector('.y-loading-mask'));
        document.body.removeChild(vm.$el);
        done();
      });
    });
    ...
    it('text', done => {
      ...
      Vue.nextTick(() => {
        const mask = document.querySelector('.y-loading-mask');
        const text = mask.querySelector('.y-loading-text');
  ...
  describe('as a service', () => {
    it('create', () => {
      ...
      expect(document.querySelector('.y-loading-mask')).to.exist;
    });
    ...
    it('target', done => {
      ...
      let mask = document.querySelector('.y-loading-mask');
    ...
    it('fullscreen singleton', done => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          let masks = document.querySelectorAll('.y-loading-mask');
          expect(masks.length).to.equal(1);
          loadingInstance2.close();
          setTimeout(() => {
            masks = document.querySelectorAll('.y-loading-mask');
    ...
    it('text', () => {
      ...
      const text = document.querySelector('.y-loading-text');
      expect(text).to.exist;
