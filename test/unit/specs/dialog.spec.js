...
describe('Dialog', () => {
  ...
  it('create', done => {
    vm = createVue({
      template: `
        <div>
          <y-dialog :title="title" :visible="visible"></y-dialog>
        </div>
      `,
    ...
    setTimeout(() => {
      ...
      expect(vm.$el.querySelector('.y-dialog__title').textContent).to.equal('dialog test');
  ...
  it('render correct content', done => {
    vm = createVue({
      template: `
        <div>
          <y-dialog :title="title" :visible="visible">
            <span>这是一段信息</span>
    ...
    setTimeout(() => {
      const footerBtns = vm.$el.querySelectorAll('.y-dialog__footer .y-button');
      expect(vm.$el.querySelector('.y-dialog__body span').textContent).to.equal('这是一段信息');
  ...
  it('append to body', done => {
    vm = createVue({
      template: `
        <div>
          <y-dialog :title="title" append-to-body :visible="visible"></y-dialog>
        </div>
      `,
  ...
  it('open and close', done => {
    vm = createVue({
      template: `
        <div>
          <y-dialog :title="title" :visible.sync="visible">
            <span>这是一段信息</span>
          </y-dialog>
        </div>
      `,
  ...
  describe('props', () => {
    const getDialogVm = (props, options) => {
      return createVue(Object.assign({
        template: `
          <div>
            <y-dialog ${ props } :title="title" :visible="visible">
              <span>这是一段信息</span>
            </y-dialog>
          </div>
        `,
    ...
    it('fullscreen', () => {
      ...
      const dialogEl = vm.$el.querySelector('.y-dialog');
    ...
    it('top', () => {
      ...
      expect(vm.$el.querySelector('.y-dialog').style.marginTop).to.equal('100px');
    });

    it('custom-class', () => {
      vm = getDialogVm('custom-class="my-dialog"');
      expect(vm.$el.querySelector('.y-dialog').classList.contains('my-dialog')).to.true;
    });
  });
  ...
  it('click header btn', done => {
    ...
    const dialog = vm.$children[0];
    setTimeout(() => {
      dialog.$el.querySelector('.y-dialog__headerbtn').click();
      setTimeout(() => {
        expect(vm.visible).to.be.false;
        done();
      }, 500);
  ...
  it('before close', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <y-dialog :title="title" :visible="visible" :before-close="beforeClose"></y-dialog>
        </div>
      `,
  ...
  it('destroyOnClose', async() => {
    vm = createVue({
      template: `
        <div>
          <y-dialog :title="title" :visible.sync="visible" destroy-on-close>
            <input />
          </y-dialog>
        </div>
      `,
