...
describe('Switch', () => {
  ...
  it('create', () => {
    ...
    const core = vm.$el.querySelector('.y-switch__core');
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)');
    expect(core.style.width).to.equal('100px');
    expect(vm.$el.querySelector('.y-switch__label--left').querySelector('span').textContent).to.equal('off');
  });

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'y-icon-check',
      inactiveIconClass: 'y-icon-close'
    });

    const icon = vm.$el.querySelector('.y-switch__label--left').querySelector('i');
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <y-switch disabled v-model="value"></y-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.y-switch__core').click();
    Vue.nextTick(() => {
  ...
  it('expand switch value', done => {
    vm = createVue({
      template: `
        <div>
          <y-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></y-switch>
        </div>
      `,
      data() {
        return {
