...
describe('Transfer', () => {
  ...
  const createTransfer = (props, opts) => {
    return createVue(Object.assign({
      template: `
        <y-transfer :data="testData" ref="transfer" ${props}>
        </y-transfer>
      `,
  ...
  it('filterable', done => {
    ...
    const leftList = transfer.$el.querySelector('.y-transfer-panel').__vue__;
    leftList.query = '1';
    setTimeout(_ => {
  ...
  it('customize', () => {
    ...
    const label = transfer.querySelector('.y-transfer-panel__header .y-checkbox__label');
    expect(label.innerText.indexOf('表1') > -1).to.true;
    expect(transfer.querySelector('.y-transfer-panel__list .y-checkbox__label span').innerText).to.equal('1 - 备选项 1');
  ...
  it('check', () => {
    ...
    const leftList = vm.$refs.transfer.$el.querySelector('.y-transfer-panel').__vue__;
  ...
  describe('target order', () => {
    it('original(default)', done => {
      ...
      setTimeout(() => {
        transfer.addToRight();
        setTimeout(() => {
          const targetItems = [].slice.call(vm.$el.querySelectorAll('.y-transfer__buttons + .y-transfer-panel .y-transfer-panel__body .y-checkbox__label span'));
