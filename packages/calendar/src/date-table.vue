<script>
import fecha from 'yun-ui-pc/src/utils/date';
import { range as rangeArr, getFirstDayOfMonth, getPrevMonthLastDays, getMonthDays, getI18nSettings, validateRangeInOneMonth } from 'yun-ui-pc/src/utils/date-util';
        ...
        <tbody>
          {
            this.rows.map((row, index) => <tr
              class={{
                'y-calendar-table__row': true,
                'y-calendar-table__row--hide-border': index === 0 && this.hideHeader
              }}
              key={index}>
              {
