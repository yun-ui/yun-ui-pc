...
```javascript
import Vue from 'vue'
import YStep from 'element-steps'
...
```javascript
import Vue from 'vue'
import { YSteps, YStep } from 'element-steps'

Vue.component('y-steps', YSteps)
Vue.component('y-step', YStep)
```
...
| icon | 图标 | Yun Icon 提供的图标，如果要使用自定义图标可以通过 slot 方式写入 | string | — |
