...
<script type="text/babel">
  import YInputNumber from 'yun-ui-pc/packages/input-number';
    ...
    methods: {
      ...
      setValues() {
        if (this.min > this.max) {
          console.error('[Yun Error][Slider]min should not be greater than max.');
          return;
        }
        ...
        if (this.range && Array.isArray(val)) {
          if (val[1] < this.min) {
            ...
            if (this.valueChanged()) {
              this.dispatch('YFormItem', 'el.form.change', [this.minValue, this.maxValue]);
    ...
    computed: {
      stops() {
        ...
        if (this.step === 0) {
          ...
          console.warn('[Yun Warn][Slider]step should not be 0.');
          return [];
        }
