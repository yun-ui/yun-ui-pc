...
import { isFunction } from 'yun-ui-pc/src/utils/types';
...
export default {
  ...
  methods: {
    ...
    getRows() {
      const children = ((this.$slots.default || []).filter(vnode => vnode.tag &&
            vnode.componentOptions && vnode.componentOptions.Ctor.options.name === 'YDescriptionsItem'));
  ...
  render() {
    ...
    return (
      <div class="y-descriptions">
        {
          (title || extra || $slots.title || $slots.extra)
            ? <div class="y-descriptions__header">
              <div class="y-descriptions__title">
                { $slots.title ? $slots.title : title}
              </div>
              <div class="y-descriptions__extra">
        ...
        <div class="y-descriptions__body">
          <table class={['y-descriptions__table', {'is-bordered': border}, descriptionsSize ? `y-descriptions--${descriptionsSize}` : '']}>
            {rows.map(row => (
